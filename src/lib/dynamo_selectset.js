...
export default (Layout, Picker, ProgressBar, Container) => {
	...
	class DynamoSelectSet extends Component {
		constructor(props) {
			...
			let items = this.getPickerItemsById(this.props.value),
				containerValues = (items || []).reduce((sum, x) => {
		...
		componentWillReceiveProps(next) {
			if (
				(next.value && !_.isEqual(next.value, this.props.value)) ||
				(next.items &&
					next.items.length &&
		...
		respondToPickerValueChanged(v, items = this.props.items) {
			let _items = this.getPickerItemsById(v, items),
				pickerHasElements =
					items &&
					items.length &&
					items.filter(x => x.id == v).length,
				pickerValue = (pickerHasElements && v) || null;
			if (this._mounted) {
				this.setState({
					//pickerValue,
					items: _items
