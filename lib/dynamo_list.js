...
export default (Layout, Button, List, Modal, ErrorText, Container) => {
	...
	class DynamoList extends Component {
		...
		isLessThanMaxLength(element) {
			return (
				(this.state.items &&
					...
					this.state.items.length <= element.config.max) ||
		...
		getItemTemplate() {
			...
			let itemTemplate = this.clone(this.props.args.itemTemplate);
			...
			return (
				...
				<Layout value={this.props.label}>
					...
					<Modal
						...
						done={result => this.closeModal(result)}
					/>
				</Layout>
				/*jshint ignore:end */
			);
