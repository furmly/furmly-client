...
export default (
	...
	const equivalent = function(arr, arr2) {
		...
		return _.isEqualWith(arr, arr2, _.isMatch);
	};
	...
	class DynamoList extends Component {
		...
		componentWillReceiveProps(next) {
			...
			if (!equivalent(next.dataTemplate, this.props.dataTemplate)) {
				if (this._mounted) {
		...
		componentDidMount() {
			...
			let equal = equivalent(this.state.items, this.props.dataTemplate);
