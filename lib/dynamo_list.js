...
export default (
	...
	const mapStateToProps = (_, initialProps) => (state, ownProps) => {
		return {
			confirmation: state.confirmationResult&& state.confirmationResult[ownProps.component_uid],
	...
	class DynamoList extends Component {
		...
		componentWillReceiveProps(next) {
			if (
				...
				this.props.confirmation.index &&
				this.state.items.length
			) {
				this.state.items.splice(this.props.confirmation.index, 1);
			}
