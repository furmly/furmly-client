...
export default (Link, NavigationActions) => {
	...
	class DynamoNav extends Component {
		...
		go() {
			let params = null;
			if (this.props.args.config.value && this.props.args.config.value.indexOf("|") !== -1) {
				//parameters to pass on.
				let key_value = this.props.args.config.value.split("|");
			...
			switch (this.props.args.type) {
				...
				case DynamoNav.NAV_TYPE.DYNAMO:
					const setParamsAction = NavigationActions.setParams({
						params: { id: this.props.args.config.value, fetchParams: params },
						key: "Dynamo"
					});
