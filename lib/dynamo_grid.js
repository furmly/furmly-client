...
import { runDynamoProcessor, getMoreForGrid, filterGrid, getSingleItemForGrid, showMessage } from "./actions";
...
export default (Layout, List, ItemView, Header, ProgressBar, CommandsView, NavigationActions, Container) => {
	if (
	...
	const mapDispatchToProps = dispatch => {
		return {
			...
			getSingleItem: (id, args, key) => dispatch(getSingleItemForGrid(id, args, key)),
	...
	const mapStateToProps = (_, initialProps) => state => {
		...
		return {
			...
			commandProcessed: state.dynamo[initialProps.component_uid + DynamoGrid.commandResultViewName()],
			processed: state.dynamo[initialProps.component_uid + DynamoGrid.itemViewName()]
		};
	};
	class DynamoGrid extends Component {
		...
		getItemsFromSource(filter = this.props.filter, methodName = "run", extra) {
			this.props[methodName](
				this.props.args.source,
				Object.assign({}, JSON.parse(this.props.args.gridArgs || "{}"), { count: this.state.count, full: true }, filter ? { query: filter } : {}, extra || {}),
				...
				() => {
					...
					if (!id) {
						return console.error(
							"done  was called on a grid view in " + this.props.args.mode + " and it does not have a processor for it. \n" + JSON.stringify(this.props, null, " ")
						);
					}
		...
		showItemView(mode, args, skipFetch) {
			...
			if (this.props.args.extra)
				switch (mode) {
					...
					case ITEM_MODES.EDIT:
						...
						if (this.props.args.extra.fetchSingleItemProcessor && !skipFetch) {
			...
			if (!template || !Array.prototype.isPrototypeOf(template)) {
				return console.error(
					"showItemTemplate was called on a grid view in " + this.props.args.mode + " and it does not have a template. \n" + JSON.stringify(this.props, null, " ")
				);
			}
			...
			switch (command.commandType) {
				...
				case "PROCESSOR":
					this.props.run(
						...
						Object.assign({}, JSON.parse(this.props.args.gridArgs || "{}"), item),
						this.props.component_uid + DynamoGrid.commandResultViewName()
					);
					break;
			}
		}
		...
		render() {
			...
			return (
				<Layout>
					...
					/>
					<ItemView
						visibility={args.mode == GRID_MODES.CRUD && this.state.showItemView}
						done={this.done}
					...
					/>
					<CommandsView visibility={this.state.showCommandsView} close={this.closeCommandView} commands={this.props.args.commands} execCommand={this.execCommand} />
				</Layout>
			);
		}
	}
