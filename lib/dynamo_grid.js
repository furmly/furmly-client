...
export default (
	...
	const mapStateToProps = (_, initialProps) => state => {
		...
		return {
			...
			filter: result && (result.filterTemplate || result.filter || null),
			...
			fetchingFilterTemplate: result && result.fetchingFilterTemplate,
	...
	class DynamoGrid extends Component {
		constructor(props) {
			super(props);
			this.state = { validator: {}, showItemView: false, count: this.props.args.pageCount||5 };
		...
		render() {
			let args = this.props.args,
				header = this.props.filter ? (
					<Header filter={() => this.filter()}>
						<Container
							elements={this.props.filter}
