...
const socket = openSocket(CHAT_URL);
    ...
    meta: (action, state, res) => {
      if (customType && res.status !== 401)
        dispatch(
          displayMessage(
            res.statusText ||
              (res.headers && res.headers.map && res.headers.map.errormessage && res.headers.map.errormessage[0]) ||
      ...
      return meta ? meta(action, state, res) : res.statusText || "An unknown error has occurred";
    }
  };
}
...
export function runDynamoProcessorAndParseUI(id, args, key) {
  return runDynamoProcessor(id, args, key, {
    returnsUI: true
  });
}

export function runDynamoProcessor(id, args, key, { requestCustomType, resultCustomType, errorCustomType, returnsUI } = {}) {
      ...
      [CALL_API]: preDispatch(
        {
          ...
          types: [
            ...
            defaultError(dispatch, errorCustomType || ACTIONS.DYNAMO_PROCESSOR_FAILED, () => key)
          ],
          method: "POST",
          headers: {
            Accept: "application/json",
    ...
    socket.emit("login", preLogin(credentials, getState()), msg => {
      if (msg.error) {
        if (!msg.isSignedUp && credentials.handle) {
          dispatch(showMessage("An error occurred while contacting the chat server. Please retry"));
        }
      ...
      return dispatch({ type: ACTIONS.LOGGED_IN_CHAT, payload: msg.message, meta: credentials });
    });
  };
}

export function sendMessage(type, msg) {
  return emit(type, msg, { requestType: ACTIONS.SEND_CHAT, resultType: ACTIONS.SENT_CHAT, errorType: ACTIONS.FAILED_TO_SEND_CHAT });
}
export function createGroup(msg) {
  return emit("create group", msg, { requestType: ACTIONS.CREATE_GROUP, resultType: ACTIONS.CREATED_GROUP });
}
...
export function searchForHandle(handle) {
  return emit("search for handle", handle, { requestType: ACTIONS.SEARCH, resultType: ACTIONS.FOUND, errorType: ACTIONS.NOT_FOUND });
}
  ...
  return (dispatch, getState) => {
    ...
    socket.emit(type, message, result => {
      if (result.error) {
        ...
        return dispatch(showMessage(result.message)), errorType && dispatch({ type: errorType, payload: result });
      }
      dispatch({ type: resultType, payload: result && result.message, meta: message });
    });
  };
}
