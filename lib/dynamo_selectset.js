...
export default (Layout, Picker,ProgressBar, Container) => {
	...
	const mapDispatchToProps=(dispatch)=>{
		return {
			getItems:(id,args,key)=>dispatch(runDynamoProcessor(id,args,key))
		}
	}
	const mapStateToProps=(_,initialProps)=state=>{
		return {
			busy: (!initialProps.args.items||!initialProps.args.items.length) && initialProps.args.processor && !state.dynamo[initialProps.component_uid],
			items:state.dynamo[initialProps.component_uid]||initialProps.args.items
		}
	}
	...
	class DynamoSelectSet extends Component {
		...
		componentWillReceiveProps(next) {
			if (next.value && next.value !== this.state.pickerValue) {
				setTimeout(() => this.setState({ pickerValue: next.value,items:this.getPickerItemsById(next.value) }), 0);
			}
		}
		componentDidMount(){
			if(this.props.args.processor){
				this.props.getItems(this.props.args.processor,JSON.parse(this.props.args.processorArgs||'{}'),this.props.component_uid)
			}
		}
		getPickerValue() {
		...
		getPickerItemsById(v) {
			if (v) return this.props.args.items.filter(x => x.id == v)[0].elements || [];
			return [];
		}
		...
		render() {
         ...
         if(!this.props.items||!this.props.items.length){
			...
			let initialElementsData = this.props.args.path ? (this.props.extra ? this.props.extra[this.props.args.path] : {}) : this.props.extra;
			return (
				<Layout
	...
	return connect(null,mapDispatchToProps)(DynamoSelectSet);
};
