...
export default function(state = {}, action) {
	switch (action.type) {
		...
		case ACTIONS.GET_ITEM_TEMPLATE:
			return Object.assign({}, state, {
				[action.meta]: getItemTemplate(state[action.meta], action)
			});
		case ACTIONS.GOT_ITEM_TEMPLATE:
			configureTemplates(state, action);
			return Object.assign({}, state, {
				[action.payload.key]: gotItemTemplate(
					state[action.payload.key],
					action
				)
			});
		case ACTIONS.FAILED_TO_GET_ITEM_TEMPLATE:
			return Object.assign({}, state, {
				[action.meta]: failedToGetItemTemplate(
					state[action.meta],
					action
				)
			});
...
function getItemTemplate(state = {}, action) {
	return Object.assign({}, state, { gettingTemplate: true });
}
function gotItemTemplate(state = {}, action) {
	return Object.assign({}, state, {
		itemTemplate: action.payload.data,
		gettingTemplate: false
	});
}
function failedToGetItemTemplate(state = {}, action) {
	return Object.assign({}, state, { gettingTemplate: false });
}
