...
export default function(state = {}, action) {
	switch (action.type) {
		...
		case ACTIONS.DYNAMO_PROCESS_RAN:
			...
			if (state.description.steps.length == 1 || (state.currentStep || 0) + 1 > state.description.steps.length - 1) {
				return {
					busy: false,
					completed: true,
					message: (typeof action.payload.data == "object" && action.payload.data.message) || null
				};
			}
			...
			currentState.value =
				typeof action.payload.data == "object" && typeof action.payload.data.message == "object" && action.payload.data.message;
		...
		case ACTIONS.GOT_SINGLE_ITEM_FOR_GRID:
			return Object.assign({}, state, {
				[action.payload.key]: gotSingleItemForGrid(state[action.payload.key], action)
			});
		case ACTIONS.ERROR_WHILE_GETTING_SINGLE_ITEM_FOR_GRID:
			return Object.assign({}, state, {
				[action.payload.key]: errorWhileGettingSingleItemForGrid(state[action.payload.key], action)
			});
		case ACTIONS.ERROR_WHILE_FETCHING_GRID:
			return Object.assign({}, state, {
				[action.payload.key]: failedToFetchGrid(state[action.payload.key])
			});
		...
		case ACTIONS.FETCHED_PROCESS:
			...
			let fetchedDescription = Object.assign({}, action.payload.data.description);
			return {
		...
		case ACTIONS.GET_ITEM_TEMPLATE:
			return Object.assign({}, state, {
				[action.meta]: getTemplate("gettingTemplate", state[action.meta], action)
			});
		case ACTIONS.GOT_ITEM_TEMPLATE:
			configureTemplates(state, action);
			return Object.assign({}, state, {
				[action.payload.key]: gotTemplate("gettingTemplate", "itemTemplate", state[action.payload.key], action)
			});
		case ACTIONS.FAILED_TO_GET_ITEM_TEMPLATE:
			return Object.assign({}, state, {
				[action.meta]: failedToGetTemplate("gettingTemplate", state[action.meta], action)
			});
		case ACTIONS.GET_FILTER_TEMPLATE:
			return Object.assign({}, state, {
				[action.meta]: getTemplate("gettingFilterTemplate", state[action.meta], action)
			});
		case ACTIONS.GOT_FILTER_TEMPLATE:
			if (action.error) {
				return Object.assign({}, state, {
					[action.meta]: failedToGetTemplate("gettingFilterTemplate", state[action.meta], action)
				});
			}
			configureTemplates(state, action);
			return Object.assign({}, state, {
				[action.payload.key]: gotTemplate("gettingFilterTemplate", "filterTemplate", state[action.payload.key], action)
			});
...
function runThroughObj(conditions, data, result = {}, parent = null) {
	Object.keys(data).forEach(key => {
		...
		if (data[key] && typeof data[key] == "object") return runThroughObj(conditions, data[key], result, data);
	});
	return result;
}
