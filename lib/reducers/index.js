...
export default function(state = {}, action) {
	switch (action.type) {
		...
		case ACTIONS.DYNAMO_PROCESS_RAN:
			...
			if (state.description.steps.length == 1 || (state.currentStep || 0) + 1 > state.description.steps.length - 1) {
				return {
					busy: false,
					completed: true,
					message: typeof action.payload.data == "object" ? action.payload.data.message : action.payload.data
				};
			}
		...
		case ACTIONS.DYNAMO_PROCESSOR_RAN:
			return Object.assign({}, state, {
				[action.payload.key]: !action.payload.returnsUI ? action.payload.data :getTemplatesAndAddComponentUid(action.payload.data)
			});
