	...
	class DynamoMessenger extends Component {
		constructor(props) {
			...
			this.state = {
				...
				showModal: false,
				_panes: [
					{ title: "Chat", icon: "comment-multiple-outline", render: this.renderChat },
					{ title: "Pending Invites", icon: "account-plus", render: this.renderPendingInvites }
				]
			};
			this._chatLayoutCommands = [{ title: "New Group", uid: "NEW_GROUP" }];
			this._chatCommands = [{ title: "Delete Chat", uid: "DELETE_CHAT" }];
		}
		sendMessage(contact, message) {
			this.props.send(contact.type !== "group" ? "msg" : "grpmsg", { to: contact.handle, message });
		}
		...
		componentDidMount() {
			...
			if (this.props.chat) {
				this.openChatModal(this.props.openChats[this.props.chat.contact.handle]);
			}
		}
		...
		renderChat() {
			let chats = Object.keys(this.props.openChats || {}).map(x => {
				return { handle: x, messages: this.props.openChats[x].messages };
			});
			return (
				<OpenChatsLayout>
					...
					{this.props.fetchingContacts ? (
						...
						<OpenChats
							open={handle => {
								this.props.openChat(this.props.openChats[handle]);
							}}
		...
		getMessageLayout(chat) {
			return (
				<ChatLayout info={chat.contact.handle}>
					...
					<ChatHistory messages={chat.messages} me={this.props.handle} />
					<Editor messageDelivered={this.props.messageDelivered} send={message => this.sendMessage(chat.contact, message)} />
				</ChatLayout>
			);
		}
		openChat(chat) {
					...
					this.props.busyWithContacts ? (
						...
						<ContactList
							...
							contactSelected={contact => {
								//add chat to history.
								let chat = (this.props.openChats || {})[contact.handle];
								if (!chat) this.props.addToOpenChats((chat = { contact, messages: [] }));
								this.openChat(chat);
							}}
						/>
					),
			...
			this.setState({
				modalTemplate: () => (
					<AddNewContact
						...
						sendFriendRequest={handle => this.props.sendFriendRequest(handle)}
					/>
				),
				showModal: true
			});
			...
			if (!this.props.handle) {
				return (
					<Login
						...
						welcomeMessage={
							(this.props.args && this.props.args.welcomeMessage) ||
							"Please enter the name with which you want to be known. Please note once saved it cannot be changed"
						}
						submit={handle => this.props.login({ username: this.props.username, handle })}
					/>
				);
			}
			return (
				<Layout
					...
					panes={this.state._panes.map(x => ({ title: x.title, icon: x.icon }))}
				>
					<Modal
